...
@pytest.mark.asyncio
async def test_help_slash_returns_slash_commands():
    from commands import help_slash

    result = await help_slash()

    for cmd in ["/link", "/unlink", "/info", "/add", "/edit", "/delete"]:
        assert cmd in result


@pytest.mark.asyncio
async def test_link_slash_returns_auth_url():
    from commands import link_slash

    with patch(
        "commands.get_auth_url", return_value="https://accounts.google.com/fake"
    ):
        result = await link_slash("12345")

    assert "https://accounts.google.com/fake" in result


@pytest.mark.asyncio
async def test_unlink_slash_success():
    from commands import unlink_slash

    with patch("commands.delete_token", return_value=True):
        result = await unlink_slash("12345")

    assert "Unlinked" in result


@pytest.mark.asyncio
async def test_unlink_slash_not_linked():
    from commands import unlink_slash
...
@pytest.mark.asyncio
async def test_info_slash_not_linked():
    from commands import info_slash

    with patch("commands.get_credentials", return_value=None):
        result = await info_slash("12345", "05.01.2026")

    assert "Not linked" in result


@pytest.mark.asyncio
async def test_info_slash_no_events_found():
    from commands import info_slash
...
@pytest.mark.asyncio
async def test_info_slash_returns_event_details():
    from commands import info_slash
...
@pytest.mark.asyncio
async def test_add_slash_success():
    from commands import add_slash
...
@pytest.mark.asyncio
async def test_edit_slash_success():
    from commands import edit_slash

    with (
        patch("commands.get_credentials", return_value=MagicMock()),
        patch("commands.edit_event", return_value=MagicMock()),
    ):
        result = await edit_slash("12345", "001", "title", "New Title")
...
@pytest.mark.asyncio
async def test_delete_slash_success():
    from commands import delete_slash

    with (
        patch("commands.get_credentials", return_value=MagicMock()),
        patch("commands.delete_event", return_value=True),
    ):
        result = await delete_slash("12345", "001")
